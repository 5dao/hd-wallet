...
func testVectorKeyPairs(t *testing.T, vector testMasterKey) {
	...
	TEqual(t, vector.privKey, privKey.String())
	TEqual(t, vector.pubKey, pubKey.String())
	...
	for _, testChildKey := range vector.children {
		...
		// Assert correctness
		TEqual(t, testChildKey.privKey, privKey.String())
		TEqual(t, testChildKey.pubKey, pubKey.String())
...
func TestPublicParentPublicChildDerivation(t *testing.T) {
	...
	for _, child := range expectedChildren {
		...
		TEqual(t, child.hexPubKey, hex.EncodeToString(pubKey.Key))
	}
}
...
func TestCantCreateHardenedPublicChild(t *testing.T) {
	...
	_, err = key.NewChildKey(FirstHardenedChild)
	TEqual(t, ErrHardnedChildPublicKey, err)
	_, err = key.NewChildKey(FirstHardenedChild + 1)
	TEqual(t, ErrHardnedChildPublicKey, err)
}
...
func assertKeySerialization(t *testing.T, key *Key, knownBase58 string) {
	...
	TEqual(t, knownBase58, serializedBase58)
	...
	TEqual(t, key, unserializedBase58)
}

func TEqual(t *testing.T, a interface{}, b interface{}) {
